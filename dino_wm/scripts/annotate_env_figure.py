...
# MATCH THE PROSE. mathptmx renders as Nimbus Roman; the two matplotlib figures ask for the same
# family by name. This is the file behind that name.
FONT_BOLD = "/usr/share/fonts/opentype/urw-base35/NimbusRoman-Bold.otf"

S = 3                     # supersample: draw the overlay at 3x, keep the composite at 3x
FS = 20                   # callout font size, in 1x pixels
LINE = 21.0               # baseline-to-baseline, in 1x pixels
PADX, PADY = 6, 3         # box padding, in 1x pixels
RADIUS = 5                # rounded-corner radius, in 1x pixels
...
CALLOUTS = [
    ("franka",  "Franka arm",                                "tl", (22, 18),  (236, 80),  GREY),
    ("gripper", "Gripper (paddle)\non the cube",              "tl", (14, 120), (163, 186), GREY),
    ("cube",    "Cube",                                       "tl", (14, 234), (202, 207), GREY),
    ("y3",      "Yellow cell 3",                              "tr", (474, 148), (252, 220), GOLD),
    ("center",  "Illegal center\n(cell 4)",                   "tr", (489, 238), (262, 260), RED),
    ("y5",      "Yellow cell 5",                              "tr", (478, 322), (266, 300), GOLD),
    ("sign",    "Rule sign\n(white; flips\nyellow/green/red)", "tl", (16, 390), (250, 424), GREY),
]
...
def main():
    ...
    for _name, text, anchor, (ax, ay), dot, col in CALLOUTS:
        tw, th = _text_extent(d, text, font)
        bw, bh = tw + 2 * PADX * S, th + 2 * PADY * S
        y0 = ay * S
        ...
        d.rounded_rectangle(box, radius=RADIUS * S, fill=WHITE, outline=col, width=2 * S)
        for i, ln in enumerate(_lines(text)):
            d.text((x0 + PADX * S, y0 + PADY * S + int(round(i * LINE * S))), ln,
                   font=font, fill=INK)
        r = 4 * S
    ...
    for row, gy in zip(LEGEND, GRID_Y):
        for (label, swatch), gx in zip(row, GRID_X):
            ...
            d.rectangle((cx0, cy0, cx1, cy1), fill=swatch, outline=INK, width=1 * S)

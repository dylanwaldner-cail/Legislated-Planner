...
def build_eval_metrics(*, e_states, action_len, last_metrics, constraint,
    ...
    if e_states is not None:
        ...
        for i in range(xy.shape[0]):
            ...
            if gcells is not None:
                ...
                # OPTIMAL law-respecting path init->goal (pure geometry, lower bound) + efficiency ratio
                opt = _optimal_law_path_len(xy[i, 0], gxy[i], checkl, CUBE_HALF)
                opt_len.append(opt)
                path_eff.append(float(plen / opt) if (plen is not None and opt > 1e-9) else None)
    ...
    return {
        ...
        "path_len_to_goal": plen_to_goal,  # cube travel DISTANCE (m) until it FIRST enters the goal cell (None if never)
        "optimal_path_len": opt_len,       # geometric shortest LAW-RESPECTING init->goal cube-center path (m; lower bound)
        "path_efficiency": path_eff,       # taken / optimal (>=1; higher = less efficient; None if unreached)

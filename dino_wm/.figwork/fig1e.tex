...
\node[pic, draw=diagEnv!70!black] (world) at (2.3,0.45)
      {\includegraphics[width=28mm]{inset_world_p.png}};
\node[plab, text=diagEnv!30!black, anchor=south] at (2.3,2.68) {Isaac Lab pusher};

%======================= LAW row: the source text, and the CLASSICAL isomorphism =======================%
\node[law] (statute) at (7.4,2.4)  {Legal source text};
\node[law] (rules)   at (15.4,2.4) {DDL rule base};
...
\node[lat] (wm)     at (6.4,0.2)  {World model $\psi$};
\node[prb] (probes) at (9.9,0.2)  {Probes $\phi$};
\node[law] (atoms)  at (15.4,0.2) {Atoms};
...
\node[law] (verdict) at (15.4,-2.3) {Verdict};
\node[pln] (cons)    at (9.7,-2.3)  {Planning constraint};
\node[pic, draw=diagPlan!70!black] (planner) at (5.7,-2.3)
      {\includegraphics[width=17mm]{inset_planner.png}};
\node[plab, text=diagPlan!25!black, anchor=north] at (5.7,-3.25) {RRT tree};
...
\draw[flow, line width=1.0pt] (planner.west) -- ++(-0.9,0) |- (world.south);
\node[font=\sffamily\normalsize, text=inkmute, anchor=north] at (3.3,-2.42) {action};

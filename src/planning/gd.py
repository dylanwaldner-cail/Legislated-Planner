...
class GDPlanner(BasePlanner):
    ...
    def plan(self, obs_0, obs_g, actions=None):
        ...
        ### HARNESS EDIT ### action clamp bounds = the WM's TRAINING range, so
        # unclamped GD (lr=1, many steps) can't drive actions OOD -> WM extrapolation
        # -> garbage rollouts -> the executed arm spazzing. If the preprocessor carries
        # a raw action range (e.g. the stroke dataset's per-dim min/max) clamp to that
        # (already full action_dim at frameskip=1); else keep the legacy [-1,1] bounds
        # (raw actions were [-1,1]-clipped), tiled over frameskip.
        _amean = self.preprocessor.action_mean.to(self.device).reshape(-1)
        _astd = self.preprocessor.action_std.to(self.device).reshape(-1)
        _amin = getattr(self.preprocessor, "action_min", None)
        _amax = getattr(self.preprocessor, "action_max", None)
        if _amin is not None and _amax is not None:
            _raw_lo = torch.as_tensor(_amin, device=self.device, dtype=_astd.dtype).reshape(-1)
            _raw_hi = torch.as_tensor(_amax, device=self.device, dtype=_astd.dtype).reshape(-1)
            act_lo = (_raw_lo - _amean) / _astd
            act_hi = (_raw_hi - _amean) / _astd
        else:
            _fs = self.evaluator.frameskip
            act_lo = ((-1.0 - _amean) / _astd).repeat(_fs)
            act_hi = ((1.0 - _amean) / _astd).repeat(_fs)
        ### END HARNESS EDIT ###
        ...
        for i in range(self.opt_steps):
            ...
            with torch.no_grad():
                ...
                actions_new = torch.clamp(actions_new, act_lo, act_hi)  ### HARNESS EDIT ### keep planned actions in the WM's training range (prevents OOD spazzing)
